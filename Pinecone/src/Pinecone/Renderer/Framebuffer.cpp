...
namespace Pinecone
{
	...
	void Framebuffer::Invalidate()
	{
		...
		{
			PC_CORE_ASSERT(m_ColorAttachments.size() <= 4);
		...
		PC_CORE_ASSERT(glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE, "Framebuffer is incomplete!");
	...
	void Framebuffer::Resize(uint32_t width, uint32_t height)
	{
		...
		{
			PC_CORE_WARN("Attempted to rezize framebuffer to {0}, {1}", width, height);
			return;
		}
	...
	{
		PC_CORE_ASSERT(attachmentIndex < m_ColorAttachments.size());
