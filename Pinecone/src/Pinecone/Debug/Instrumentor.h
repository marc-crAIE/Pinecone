	...
	public:
		...
		void BeginSession(const std::string& name, const std::string& filepath = "results.json")
		{
			...
			if (m_CurrentSession)
			{
				...
				{
					PC_CORE_ERROR("Instrumentor::BeginSession('{0}') when session '{1}' already open.", name, m_CurrentSession->Name);
				}
				InternalEndSession();
			}
...
#define PC_PROFILE_BEGIN_SESSION(name, filepath) ::Pinecone::Instrumentor::Get().BeginSession(name, filepath)
#define PC_PROFILE_END_SESSION() ::Pinecone::Instrumentor::Get().EndSession()
#define PC_PROFILE_SCOPE_LINE2(name, line) constexpr auto fixedName##line = ::Pinecone::InstrumentorUtils::CleanupOutputString(name, "__cdecl ");\
												   ::Pinecone::InstrumentationTimer timer##line(fixedName##line.Data)
